...
void NachOS_Write() {		// System call 6
   ...
   int return_value = synchConsole->Write(char_buffer, size);
...
void NachOS_Read() {		// System call 7
   ...
   char char_buffer[Char_Size_Of_Array];
   while (counter < size) {
      machine->WriteMem(buffer_pointer, 1, char_buffer[counter]);
      counter++;
      buffer_pointer++;
   }
   int return_value = synchConsole->Read(char_buffer, size);
   machine->WriteRegister(2, return_value);
}
...
void NachOS_Close() {		// System call 8
   ...
   int return_value = synchConsole->Close(id);
   machine->WriteRegister(2, return_value);
}
